...
const SSH_KEY_PATH = process.env.SSH_KEY_PATH || path.join(process.env.HOME || "~", ".ssh", "hetzner")
const REMOTE_BASE_DIR = process.env.REMOTE_BASE_DIR || "/var/www/activityfair"
const SITE_BASE_URL = process.env.SITE_BASE_URL || "https://activityfair.tjdev.club"
      ...
      if (!smtpHost || !smtpUser || !smtpPass) {
        ...
        const info = await transporter.sendMail({
          ...
          html: `<p>Hey ${displayName},</p>
                 <p>We've set up your Activity Fair submission. You can view it here:</p>
                 <p><a href="${siteUrl}">${siteUrl}</a></p>
                 <p>Quick heads up: sign up for our 8th period — spots fill up fast.</p>
...
async function waitForStableFile(filePath: string, timeoutMs = 10000, stableCycles = 3): Promise<void> {
